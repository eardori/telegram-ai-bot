...
bot.callbackQuery(/^nsfw_consent_yes:(.+):(.+)$/, async (ctx) => {
  try {
    ...
    // Grant consent
    const { nsfwSafetyService } = await import('../../src/services/nsfw-safety');
    ...
    const { data: nsfwTemplates, error } = await supabase
      .from('prompt_templates')
      .select('*')
      .eq('category', 'nsfw')
      .eq('is_active', true)
    ...
    await ctx.reply(
      `🎨 **nsfw 스타일** (${nsfwTemplates.length}개)\n\n원하는 스타일을 선택하세요:`,
      { reply_markup: keyboard }
    );
...
bot.callbackQuery(/^p:([a-z0-9]+)$/, async (ctx) => {
  try {
    ...
    // Send processing message
    const processingMsg = await ctx.reply(
      `✨ **${templateWithParams.template_name_ko}** 편집 중...\n\n` +
          ...
          if (referrerId) {
            try {
              await bot.api.sendMessage(
                referrerId,
                formatReferrerNotification(username || `사용자 ${userId}`, result.referrerReward || 10)
              );
            } catch (error) {
