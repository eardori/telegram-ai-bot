...
function getPrivateWelcomeMessage(firstName: string, language: string): string {
...
function getGroupWelcomeMessage(firstName: string, language: string): string {
  const messages: Record<string, string> = {
    en: `
...
async function trackBotStart(ctx: BotContext): Promise<void> {
  try {
    // Track bot start event in analytics
    // This could be sent to an analytics service or stored in database
    ...
    // Don't fail the command if analytics tracking fails
  }
}