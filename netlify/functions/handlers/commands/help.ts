...
export function setupHelpCommand(bot: Bot): void {
  bot.command('help', async (ctx) => {
    ...
    try {
      ...
      await ctx.reply(
        '❌ Sorry, I encountered an error loading help. Please try again later.',
        { reply_to_message_id: ctx.message?.message_id },
      );
    }
  });
...
export async function handleHelpCallback(params: string[], ctx: BotContext): Promise<void> {
...
function getHelpMessage(language: string, isPrivateChat: boolean, isAdmin: boolean): string {
...
function getQuickStartMessage(language: string, isPrivateChat: boolean): string {
  const messages: Record<string, string> = {
    en: `
...
function getCommandsMessage(language: string, isPrivateChat: boolean, isAdmin: boolean): string {
...
function getSettingsHelpMessage(language: string, isAdmin: boolean): string {
  const messages: Record<string, string> = {
    en: `
...
function getAdvancedHelpMessage(language: string, isAdmin: boolean): string {
  const messages: Record<string, string> = {
    en: `
...
function getFAQMessage(language: string): string {
  const messages: Record<string, string> = {
    en: `
...
async function isUserAdmin(ctx: BotContext): Promise<boolean> {
  try {
    ...
    // This is a simplified check - in production you'd check with Telegram API
