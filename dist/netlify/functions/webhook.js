...
// Import i18n system
const i18n_helper_1 = require("../../src/utils/i18n-helper");
...
async function getHelpMessage(lang = 'ko') {
    try {
        const versionInfo = await (0, version_manager_1.getVersionInfoForHelp)();
        const msg = (0, i18n_helper_1.t)('helpMain', lang);
        const features = (0, i18n_helper_1.t)('helpFeatures', lang);
        const commands = (0, i18n_helper_1.t)('helpCommands', lang);
        const support = (0, i18n_helper_1.t)('helpSupport', lang);
        if (lang === 'en') {
            return `${msg}
...
• 📸 인물 스타일링 (프로필, 증명사진 등)
• 🎮 게임/애니메이션 (GTA, 애니메이션 등)
• 🛠️ 이미지 편집 (배경/의상/표정 변경)
• ✨ 창의적 변환 (미술관, 앨범 커버 등)

✨ **파라미터 선택형 템플릿:**
• 🌍 **배경 변경** - 6가지 배경 (해변, 도심, 우주 등)
• 👗 **의상 변경** - 6가지 의상 (정장, 드레스, 한복 등)
• 😊 **표정 변경** - 5가지 표정 (웃음, 진지, 신비로운 등)
...
🎁 **친구 초대 보너스:**
• /referral - 초대 코드 및 통계 확인
• /enter_code - 추천 코드 입력
• 친구 1명 초대: 양쪽 각 10 크레딧
• 마일스톤 달성: 5명(+20), 10명(+50), 25명(+150), 50명(+500+VIP)
    ...
    try {
        ...
        if (editResult.success && (editResult.outputUrl || editResult.outputFile)) {
            ...
            if (testCallbackData.length > 64) {
                ...
                actionKeyboard = new grammy_1.InlineKeyboard()
...
bot.callbackQuery(/^feedback:(like|dislike):([^:]+):(.+)$/, async (ctx) => {
    try {
        ...
        // Get template name
        const { data: template } = await supabase_1.supabase
            .from('prompt_templates')
...
bot.command('start', async (ctx) => {
    ...
    try {
        ...
        const { data: userData } = await supabase_1.supabase
            ...
            .eq('id', userId)
            .single();
        const lang = (0, i18n_helper_1.getUserLanguage)(ctx, userData);
...
bot.command('language', async (ctx) => {
    ...
    try {
        ...
        const { data: userData } = await supabase_1.supabase
            ...
            .eq('id', userId)
            .single();
        const currentLang = (0, i18n_helper_1.getUserLanguage)(ctx, userData);
        const msg = (0, i18n_helper_1.t)('selectLanguage', currentLang);
...
bot.callbackQuery(/^lang:(ko|en)$/, async (ctx) => {
    try {
        ...
        const langName = (0, i18n_helper_1.getLanguageDisplayName)(selectedLang);
        const successMsg = (0, i18n_helper_1.t)('languageChanged', selectedLang)(langName);
...
bot.command('admin', async (ctx) => {
    try {
        ...
        if (subcommand === 'dashboard' || !subcommand) {
            ...
            const { data: overallStats, error: statsError } = await supabase_1.supabase
                .from('v_template_feedback_stats')
                .select('*')
                .order('total_feedback', { ascending: false })
                .limit(10);
            // Get recent feedback (last N days)
            const { data: recentStats, error: recentError } = await supabase_1.supabase
                .rpc('get_feedback_summary', { p_days: period });
            // Get low satisfaction alerts
            const { data: alerts, error: alertsError } = await supabase_1.supabase
            ...
            await ctx.reply(`❌ 알 수 없는 관리자 명령어입니다.\n\n` +
                ...
                `자세한 내용은 \`/help\`를 입력하세요.`, { parse_mode: 'Markdown' });
        }
    }
    catch (error) {
...
bot.command('referral', async (ctx) => {
    ...
    try {
        ...
        const { data: hasReferrer } = await supabase_1.supabase
            .from('referrals')
            .select('id')
