...
async function callClaudeVisionAPI(prompt, imageData, mediaType) {
    try {
        ...
        if (response.ok) {
            ...
            throw new Error(data.error?.message || `Vision API 오류: ${response.status} ${response.statusText}`);
        }
    }
    catch (error) {
...
bot.on('message:text', async (ctx) => {
    ...
    if (ctx.message.reply_to_message?.photo) {
        ...
        try {
            ...
            if (!imageResponse.ok)
                throw new Error('Telegram에서 이미지를 다운로드하지 못했습니다.');
            const imageBuffer = await imageResponse.arrayBuffer();
...
Output Format: Respond ONLY with the ready-to-use English prompt for the image generation model. Do not include any other text, explanations, or quotation marks.`;
            ...
            await ctx.replyWithPhoto(new grammy_1.InputFile(imageBufferNew, `edited_${Date.now()}.png`), {
            ...
            await handleError(ctx, error, '이미지 수정', thinkingMessage);
        }
        return;
    }
...
✨ **편집된 이미지입니다!**`;
                await ctx.replyWithPhoto(new grammy_1.InputFile(Buffer.from(imageResult.imageData, 'base64')), {
                    caption: caption
                });
...
✨ 어떤 그림을 그려드릴까요?`);
            ...
            const processingMsg = await (0, prompt_manager_1.getSystemMessage)('dobby_processing_image', {
