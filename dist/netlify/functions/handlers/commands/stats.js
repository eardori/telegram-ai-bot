...
exports.handleStatsCallback = handleStatsCallback;
    ...
    bot.command('stats', async (ctx) => {
        ...
        try {
            ...
            if (statsType === 'admin' && !isAdmin) {
                await ctx.reply('🚫 Admin statistics are only available to administrators.', { reply_to_message_id: ctx.message?.message_id });
                return;
            }
            ...
            await ctx.reply('❌ Sorry, I encountered an error loading statistics. Please try again later.', { reply_to_message_id: ctx.message?.message_id });
        }
    });
}
async function handleStatsCallback(params, ctx) {
...
async function generateStats(ctx, type, isPrivateChat) {
    if (!ctx.chat) {
...
async function generateOverviewStats(messages, userActivity, chatStats) {
...
async function generateActivityStats(messages, timeRange) {
...
async function generateContentStats(messages) {
...
async function generateEngagementStats(messages) {
...
function formatStatsMessage(stats, type, isPrivateChat) {
    switch (type) {
        case 'activity':
...
function formatOverviewStats(overview, isPrivateChat) {
    return `
...
${overview.topUsers.map((user, i) => `${i + 1}. ${user.name}: <code>${user.count}</code> (${user.percentage}%)`).join('\n')}
...
${activity.dailyActivity.map(day => `${day.day}: ${'█'.repeat(Math.round(day.count / Math.max(...activity.dailyActivity.map(d => d.count)) * 10))} ${day.count}`).join('\n')}

<b>📈 Weekly Trends</b>
${activity.weeklyTrends.map(week => `${week.week}: ${week.count} messages ${week.change > 0 ? '📈' : '📉'} ${Math.abs(week.change)}%`).join('\n')}

<b>🎯 Activity Score</b>
${totalActivity > 100 ? '🔥 Very Active' : totalActivity > 50 ? '📈 Active' : '😴 Quiet'}
  `.trim();
}
function formatContentStats(content) {
    return `
...
<b>🔤 Most Used Words</b>
${content.wordCloud.slice(0, 10).map((word, i) => `${i + 1}. <code>${word.word}</code> (${word.count})`).join('\n')}

<b>🌍 Language Distribution</b>
${Object.entries(content.languageDistribution).map(([lang, percent]) => `${lang.toUpperCase()}: ${percent}%`).join(' • ')}

<b>🏷️ Popular Topics</b>
${content.topTopics.slice(0, 5).map((topic, i) => `${i + 1}. ${topic}`).join('\n')}
  `.trim();
}
function formatEngagementStats(engagement) {
    return `
...
<b>📈 Active Threads</b>
${engagement.mostActiveThreads.map(thread => `• <b>${thread.topic}</b>: ${thread.messages} messages, ${thread.participants} participants`).join('\n')}

<b>👥 User Interactions</b>
${engagement.userInteractions.map(interaction => `• ${interaction.from} ↔ ${interaction.to}: ${interaction.count} interactions`).join('\n')}
...
function formatSummaryStats(summaries) {
    return `
...
<b>📈 Growth Trends</b>
${stats.activity.weeklyTrends.map(trend => `• ${trend.week}: ${trend.count} messages (${trend.change > 0 ? '+' : ''}${trend.change}%)`).join('\n')}
...
function getStatsKeyboard(currentType, isAdmin, isPrivateChat) {
    const keyboard = [
        [
            {
...
function generateMockStats(type, isPrivateChat) {
    return {
        overview: {
            totalMessages: 1250,
            totalUsers: 25,
...
async function trackStatsRequest(ctx, statsType) {
    try {
        const eventData = {
            event: 'stats_viewed',
            userId: ctx.user?.id,
