    ...
    bot.command('settings', async (ctx) => {
        ...
        try {
            ...
            if (!isPrivateChat && !isAdmin) {
                await ctx.reply('🚫 Only administrators can access bot settings in group chats.', { reply_to_message_id: ctx.message?.message_id });
                return;
            }
            ...
            await ctx.reply('❌ Sorry, I encountered an error loading settings. Please try again later.', { reply_to_message_id: ctx.message?.message_id });
        }
    });
}
async function handleSettingsCallback(params, ctx) {
...
function getSettingsMessage(language, isPrivateChat, ctx) {
...
function getSettingsKeyboard(isPrivateChat, ctx) {
    if (isPrivateChat) {
        return [
            [
...
function getSummarySettingsMessage(language, ctx) {
    const isPrivateChat = ctx.chat?.type === 'private';
...
function getSummarySettingsKeyboard(ctx) {
    const isPrivateChat = ctx.chat?.type === 'private';
    const settings = isPrivateChat ? ctx.user?.preferences : ctx.chat?.settings;
    const keyboard = [
        [
...
function getLanguageSettingsMessage(language) {
    const messages = {
        en: `
...
function getPrivacySettingsMessage(language, ctx) {
...
function getNotificationSettingsMessage(language, ctx) {
...
function getNotificationKeyboard(ctx) {
    const settings = ctx.user?.preferences?.notification_settings || {};
    return [
        [
            {
...
function getAdvancedSettingsMessage(language, ctx) {
    const messages = {
        en: `
...
async function handleSettingChange(action, params, ctx) {
    switch (action) {
        case 'toggle':
...
async function handleToggleSetting(settingPath, ctx) {
    try {
        const isPrivateChat = ctx.chat?.type === 'private';
...
async function handleSetSetting(settingKey, value, ctx) {
    try {
...
async function handleSaveSettings(section, ctx) {
    try {
...
function formatArraySetting(value) {
...
function getEnabledFeatures(config) {
    const features = [];
    if (config.app.features.summaryEnabled)
        features.push('Summaries');
    if (config.app.features.translationEnabled)
        features.push('Translation');
    if (config.app.features.moderationEnabled)
        features.push('Moderation');
    if (config.app.features.analyticsEnabled)
        features.push('Analytics');
    return features;
}
async function updateUserOrChatSetting(ctx, pathParts, value) {
