...
exports.getUserTotalCost = getUserTotalCost;
exports.getDailyCostSummary = getDailyCostSummary;
exports.getTotalCosts = getTotalCosts;
 ...
 */
async function getUserTotalCost(userId) {
    try {
 ...
 */
async function getDailyCostSummary(days = 7) {
    try {
        const { data, error } = await supabase_1.supabase
 ...
 */
async function getTotalCosts(startDate, endDate) {
    try {
        ...
        const total_images_processed = data.reduce((sum, log) => sum + (log.input_images || 0) + (log.output_images || 0), 0);
