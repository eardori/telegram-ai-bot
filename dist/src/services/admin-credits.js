...
exports.formatCreditGrantMessage = formatCreditGrantMessage;
exports.notifyUserCreditGrant = notifyUserCreditGrant;
...
async function grantCredits(request) {
    try {
        ...
        // Get current balance
        const { data: currentCredits } = await supabase_1.supabase
            .from('user_credits')
        ...
        const previousBalance = (currentCredits.free_credits || 0) +
            (currentCredits.paid_credits || 0) +
            (currentCredits.subscription_credits || 0);
        // Grant credits using existing addCredits function
        const result = await (0, credit_manager_1.addCredits)(userId, amount, 'free', // Grant as free credits
        `Admin grant: ${reason}`, undefined // No package key
        );
        if (!result.success) {
            return {
                success: false,
 ...
 */
async function revokeCredits(userId, amount, reason, revokedBy) {
    try {
        // Get current credits
        const { data: credits, error } = await supabase_1.supabase
            .from('user_credits')
 ...
 */
function formatCreditGrantMessage(result, username) {
    if (!result.success) {
        return `❌ **크레딧 지급 실패**\n\n${result.message}`;
    }
 ...
 */
async function notifyUserCreditGrant(bot, userId, amount, reason) {
    try {
        const message = `🎁 **크레딧 지급 알림**\n\n` +
