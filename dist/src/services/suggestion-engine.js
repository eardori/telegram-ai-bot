...
class SuggestionEngine {
    constructor() {
        ...
        this.templateMatcher = new template_matcher_1.TemplateMatcher();
    }
    /**
     * Generate edit suggestions
     */
    async generateSuggestions(analysis, userHistory, maxSuggestions = 5) {
        ...
        const rankedTemplates = await this.rankTemplates(matchedTemplates, analysis, userHistory);
        // Convert to suggestions
        const suggestions = this.templatesToSuggestions(rankedTemplates.slice(0, this.maxSuggestions), analysis);
     ...
     */
    async rankTemplates(templates, analysis, userHistory) {
     ...
     */
    applyImageCountPrioritization(scoredTemplates, analysis) {
        if (analysis.imageCount === 1) {
            // Single image priorities
            const singleImageBoosts = {
     ...
     */
    calculateCompositeScore(template, analysis, userHistory) {
        ...
        const compositeScore = weights.base * baseScore +
     ...
     */
    calculatePreferenceScore(template, userHistory) {
        if (!userHistory || userHistory.length === 0) {
            return 0.5; // Neutral score for new users
        }
        let score = 0;
        // Check category preference
        const categoryUses = userHistory.filter(h => h.templateKey === template.templateKey);
     ...
     */
    calculateNoveltyScore(template, userHistory) {
        if (!userHistory || userHistory.length === 0) {
     ...
     */
    calculateTrendingScore(template) {
     ...
     */
    templatesToSuggestions(templates, analysis) {
     ...
     */
    generateDescription(template, analysis) {
        let description = '';
        ...
        switch (template.category) {
            ...
            case image_edit_types_1.PromptCategory.CREATIVE_TRANSFORM:
                ...
                break;
            case image_edit_types_1.PromptCategory.GAME_ANIMATION:
     ...
     */
    ensureDiversity(suggestions, allTemplates, analysis) {
        ...
        if (!hasQuickOption && suggestions.length >= 2) {
            ...
            const quickTemplates = allTemplates.filter(t => t.category === image_edit_types_1.PromptCategory.IMAGE_EDITING &&
     ...
     */
    async getUserFavorites(userId, limit = 3) {
        try {
            ...
            const templates = await Promise.all(templateIds.map(id => this.templateMatcher.getTemplateByKey(String(id))));
