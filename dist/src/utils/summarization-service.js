...
class SummarizationService {
    ...
    async generateSummary(request) {
        ...
        try {
            ...
            if (request.session_id) {
                session = await tracking_service_1.trackingService.getSession(request.session_id);
            }
            else {
            ...
            if (!session) {
                throw new tracking_types_1.TrackingError('No tracking session found to summarize', tracking_types_1.TrackingErrorCode.SESSION_NOT_FOUND);
            }
            // Get messages for the session
            const messages = await tracking_service_1.trackingService.getSessionMessages(session.id, true);
            if (messages.length === 0) {
                throw new tracking_types_1.TrackingError('No meaningful messages found to summarize', tracking_types_1.TrackingErrorCode.NO_MESSAGES_TO_SUMMARIZE);
            }
            ...
            if (!claudeResponse.success) {
                throw new tracking_types_1.TrackingError(claudeResponse.error_message || 'Failed to generate summary', tracking_types_1.TrackingErrorCode.SUMMARY_GENERATION_FAILED);
            }
            ...
            const summary = await this.saveSummary({
                ...
                summary_type: tracking_types_1.SummaryType.MANUAL,
    ...
    // ---------------------------------------------------------------------------
    async callClaudeForSummary(context) {
        const startTime = Date.now();
        try {
    ...
    buildSummaryPrompt(context) {
        const { messages, chat_title, participant_count, conversation_duration, language, include_usernames, include_timestamps, focus_on_decisions, focus_on_questions } = context;
...
도비는 주인님들의 대화를 가장 유용한 형태로 정리해드렸습니다! 🏠✨`;
    ...
    // UTILITY METHODS
    // ---------------------------------------------------------------------------
    async createPromptContext(messages, request, session) {
        ...
        const { data: chatGroup } = await supabase_1.supabase
            .from('chat_groups')
            .select('chat_title')
    ...
    async getCurrentOrRecentSession(user_id, chat_id) {
        ...
        const { data: recentSession } = await supabase_1.supabase
            .from('tracking_sessions')
            .select('*')
            .eq('user_id', user_id)
            .eq('chat_id', chat_id)
    ...
    async saveSummary(data) {
        const { data: summary, error } = await supabase_1.supabase
            .from('conversation_summaries')
            .insert({
            ...data,
            status: tracking_types_1.SummaryStatus.COMPLETED,
            delivered_to_user: false
        })
            .select()
            .single();
        if (error) {
            throw new tracking_types_1.TrackingError('Failed to save summary', tracking_types_1.TrackingErrorCode.DATABASE_ERROR, error);
        }
        return summary;
    }
    async markSessionAsSummarized(session_id, summary_id) {
    ...
    async cleanupSessionMessages(session_id) {
        try {
     ...
     */
    getDobbyTrackingResponse(action, context = {}) {
        let template = '';
        switch (action) {
            case 'started':
     ...
     */
    getDobbySummaryResponse(stage, context = {}) {
        let template = '';
        switch (stage) {
            case 'generating':
     ...
     */
    async processReadySummaries() {
        try {
            const readySessions = await tracking_service_1.trackingService.getSessionsReadyForSummary();
            let successful = 0;
            let failed = 0;
