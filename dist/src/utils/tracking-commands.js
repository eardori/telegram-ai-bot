...
exports.parseTrackingCommand = parseTrackingCommand;
exports.shouldTrackMessage = shouldTrackMessage;
exports.handleTrackingStart = handleTrackingStart;
exports.handleTrackingStop = handleTrackingStop;
exports.handleSummarize = handleSummarize;
exports.handleTrackingStatus = handleTrackingStatus;
exports.trackMessageMiddleware = trackMessageMiddleware;
exports.handleTrackingCommand = handleTrackingCommand;
 ...
 */
function parseTrackingCommand(text, ctx) {
 ...
 */
function shouldTrackMessage(text, ctx) {
    const message = ctx.message;
    if (!message || !('from' in message) || !message.from)
        return false;
    ...
    if (parseTrackingCommand(text, ctx))
        return false;
...
// =============================================================================
function isTrackingStartCommand(text) {
    const startPatterns = [
        /도비야[,\s]*(대화\s*)?추적\s*시작/,
        /도비야[,\s]*(대화\s*)?기록\s*시작/,
        /도비야[,\s]*(대화\s*)?저장\s*시작/,
...
function isTrackingStopCommand(text) {
    const stopPatterns = [
        /도비야[,\s]*(대화\s*)?추적\s*중단/,
        /도비야[,\s]*(대화\s*)?추적\s*중지/,
        /도비야[,\s]*(대화\s*)?추적\s*그만/,
 ...
 */
async function handleTrackingStart(command, ctx) {
    try {
        console.log(`🟢 Handling tracking start for user ${command.user_id} in chat ${command.chat_id}`);
        // Check if already tracking
        const currentStatus = await tracking_service_1.trackingService.getUserTrackingStatus(command.user_id, command.chat_id);
        if (currentStatus.is_tracking) {
            const response = summarization_service_1.summarizationService.getDobbyTrackingResponse('already_active', {
        ...
        // Start tracking
        const session = await tracking_service_1.trackingService.startTracking({
        ...
        const response = summarization_service_1.summarizationService.getDobbyTrackingResponse('started', {
        ...
        if (error instanceof tracking_types_1.TrackingError) {
            switch (error.code) {
                case tracking_types_1.TrackingErrorCode.SESSION_ALREADY_ACTIVE:
                    return; // Already handled above
                case tracking_types_1.TrackingErrorCode.SESSION_LIMIT_EXCEEDED:
 ...
 */
async function handleTrackingStop(command, ctx) {
    try {
        console.log(`🔴 Handling tracking stop for user ${command.user_id} in chat ${command.chat_id}`);
        // Check if currently tracking
        const currentStatus = await tracking_service_1.trackingService.getUserTrackingStatus(command.user_id, command.chat_id);
        if (!currentStatus.is_tracking) {
            const response = summarization_service_1.summarizationService.getDobbyTrackingResponse('not_active');
            await ctx.reply(response);
            return;
        }
        // Stop tracking
        const session = await tracking_service_1.trackingService.stopTracking({
            user_id: command.user_id,
            chat_id: command.chat_id,
...
💡 "도비야, 요약해줘"로 기존 데이터를 요약할 수 있습니다.`);
        ...
        // Send success response
        const response = summarization_service_1.summarizationService.getDobbyTrackingResponse('stopped', {
        ...
        let errorMessage = '알 수 없는 오류가 발생했습니다.';
        if (error instanceof tracking_types_1.TrackingError) {
            if (error.code === tracking_types_1.TrackingErrorCode.SESSION_NOT_FOUND) {
 ...
 */
async function handleSummarize(command, ctx) {
    try {
        ...
        const generatingMessage = summarization_service_1.summarizationService.getDobbySummaryResponse('generating', {
        ...
        // Generate summary
        const result = await summarization_service_1.summarizationService.generateSummary({
            user_id: command.user_id,
            chat_id: command.chat_id,
            summary_language: 'ko',
        ...
        if (result.success) {
            ...
            const response = summarization_service_1.summarizationService.getDobbySummaryResponse('completed', {
            ...
            // Send error response
            const response = summarization_service_1.summarizationService.getDobbySummaryResponse('failed', {
        ...
        if (error instanceof tracking_types_1.TrackingError) {
            switch (error.code) {
                case tracking_types_1.TrackingErrorCode.SESSION_NOT_FOUND:
                case tracking_types_1.TrackingErrorCode.NO_MESSAGES_TO_SUMMARIZE:
                    errorResponse = summarization_service_1.summarizationService.getDobbySummaryResponse('no_messages');
                    break;
                default:
                    errorResponse = summarization_service_1.summarizationService.getDobbySummaryResponse('failed', {
 ...
 */
async function handleTrackingStatus(command, ctx) {
    try {
        console.log(`📊 Handling status check for user ${command.user_id} in chat ${command.chat_id}`);
        const status = await tracking_service_1.trackingService.getUserTrackingStatus(command.user_id, command.chat_id);
 ...
 */
async function trackMessageMiddleware(ctx, next) {
    try {
        const message = ctx.message;
        ...
        await tracking_service_1.trackingService.trackMessage({
 ...
 */
async function handleTrackingCommand(command, ctx) {
    switch (command.type) {
        case 'start':
