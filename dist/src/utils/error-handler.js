...
exports.ensureChatGroupExists = ensureChatGroupExists;
exports.recoverOrphanedSessions = recoverOrphanedSessions;
exports.performConsistencyCheck = performConsistencyCheck;
exports.getUserFriendlyErrorMessage = getUserFriendlyErrorMessage;
exports.logErrorWithContext = logErrorWithContext;
exports.handleServiceDegradation = handleServiceDegradation;
 ...
 */
async function ensureChatGroupExists(ctx) {
    try {
        const chat = ctx.chat;
        if (!chat)
            return false;
...
async function recoverOrphanedSessions() {
    try {
        ...
        const { data: orphanedSessions } = await supabase_1.supabase
...
async function performConsistencyCheck() {
    try {
        ...
        const { data: inconsistentUsers } = await supabase_1.supabase
            .from('user_chat_tracking')
            .select(`
        user_id,
        chat_id,
      ...
      `)
        ...
        const { data: sessions } = await supabase_1.supabase
            .from('tracking_sessions')
            .select('id')
            .in('status', [tracking_types_1.TrackingSessionStatus.ACTIVE, tracking_types_1.TrackingSessionStatus.STOPPED]);
        if (sessions) {
            for (const session of sessions) {
                const { data: messageStats } = await supabase_1.supabase
        ...
        const { data: expiredMessages } = await supabase_1.supabase
            .from('tracked_messages')
            .select('id')
            .in('tracking_session_id', supabase_1.supabase
            .from('tracking_sessions')
            .select('id')
            .eq('status', tracking_types_1.TrackingSessionStatus.EXPIRED));
...
function getUserFriendlyErrorMessage(error, context) {
    ...
    switch (error.code) {
        ...
        case tracking_types_1.TrackingErrorCode.SESSION_EXPIRED:
        ...
        case tracking_types_1.TrackingErrorCode.SUMMARY_GENERATION_FAILED:
            return `${dobbyPrefix}❌ 요약 생성에 실패했습니다.\n💡 잠시 후 다시 시도해주세요.`;
        case tracking_types_1.TrackingErrorCode.SUMMARY_LIMIT_EXCEEDED:
            return `${dobbyPrefix}❌ 일일 요약 생성 한도를 초과했습니다.\n💡 내일 다시 시도해주세요.`;
        case tracking_types_1.TrackingErrorCode.DATABASE_ERROR:
            return `${dobbyPrefix}❌ 데이터베이스 오류가 발생했습니다.\n💡 잠시 후 다시 시도해주세요.`;
        case tracking_types_1.TrackingErrorCode.CLAUDE_API_ERROR:
            return `${dobbyPrefix}❌ AI 서비스 연결에 문제가 있습니다.\n💡 잠시 후 다시 시도해주세요.`;
        case tracking_types_1.TrackingErrorCode.RATE_LIMIT_ERROR:
            return `${dobbyPrefix}⏱️ 요청이 너무 많습니다.\n💡 잠시 기다린 후 다시 시도해주세요.`;
        case tracking_types_1.TrackingErrorCode.VALIDATION_ERROR:
 ...
 */
async function handleServiceDegradation(ctx, serviceType) {
    const responses = {
