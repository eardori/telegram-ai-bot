...
exports.bulkCreatePrompts = bulkCreatePrompts;
exports.exportPrompts = exportPrompts;
exports.getPromptStats = getPromptStats;
exports.cleanupOldUsageData = cleanupOldUsageData;
exports.duplicatePrompt = duplicatePrompt;
exports.batchUpdatePrompts = batchUpdatePrompts;
exports.searchPrompts = searchPrompts;
exports.validatePromptTemplate = validatePromptTemplate;
exports.generateUsageReport = generateUsageReport;
const supabase_1 = require("./supabase");
/**
 * Bulk create prompts from JSON configuration
 */
async function bulkCreatePrompts(prompts) {
    try {
 ...
 */
async function exportPrompts(includeInactive = false) {
    try {
 ...
 */
async function getPromptStats(days = 7) {
    try {
        const { data, error } = await supabase_1.supabase
            .from('prompt_usage')
            .select(`
 ...
 */
async function cleanupOldUsageData(olderThanDays = 30) {
    try {
 ...
 */
async function duplicatePrompt(sourceKey, newKey, newName) {
    try {
        // Get source prompt
        const { data: sourcePrompt, error: fetchError } = await supabase_1.supabase
            .from('prompts')
            .select('*')
            .eq('key', sourceKey)
            .single();
...
async function batchUpdatePrompts(updates) {
    try {
        const promises = updates.map(({ key, update }) => supabase_1.supabase
            .from('prompts')
            .update({
            name: update.name,
 ...
 */
async function searchPrompts(searchTerm, searchIn = ['name', 'template', 'description']) {
    try {
        let query = supabase_1.supabase.from('prompts').select('*');
 ...
 */
function validatePromptTemplate(template, variables = {}) {
    const errors = [];
    const warnings = [];
    try {
 ...
 */
async function generateUsageReport(days = 7) {
    try {
        ...
        if (errorPrompts.length > 0) {
            ...
            errorPrompts.forEach(stat => {
                ...
                if (stat.uniqueErrors.length > 0) {
                    ...
                    stat.uniqueErrors.forEach((error) => {
                        report += `  - ${error}\n`;
                    });
                }
                report += `\n`;
