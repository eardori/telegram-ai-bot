 ...
 */
export async function getVersionHistory(limit: number = 10): Promise<VersionHistoryEntry[]> {
  try {
    ...
    const history: VersionHistoryEntry[] = data.map((entry: any) => ({
 ...
 */
export async function storeGitCommit(
  ...
  try {
    ...
    const { data, error } = await supabase.rpc('store_git_commit', {
 ...
 */
export async function parseGitLog(gitLogOutput: string): Promise<GitCommitData[]> {
  try {
 ...
 */
export async function getFormattedVersionHistory(limit: number = 5): Promise<string> {
  try {
 ...
 */
export async function getVersionInfoForHelp(): Promise<string> {
  try {
    const latest = await getLatestVersion();
