...
interface Messages {
  ...
  aiAnalyzing: (title: string, description: string) => string;
  imageProcessing: (templateName: string) => string;
  editComplete: string;
  aiRecommendationError: string;

  // Credits
  creditsBalance: (free: number, paid: number, subscription: number, total: number) => string;
  creditsLow: (current: number, required: number) => string;
  creditsInsufficient: string;
  topUpPrompt: string;

  // Referral
  referralTitle: string;
  referralCode: (code: string) => string;
  referralLink: (link: string) => string;
  referralStats: (invites: number, credits: number) => string;
  referralNextMilestone: (milestone: string, current: number, target: number) => string;
  referralSharePrompt: string;
  ...
  errorTemplateList: string;
  errorOriginalImageNotFound: string;
  errorFileIdNotFound: string;
  errorRetry: string;

  // Buttons
  btnEditAgain: string;
  btnBackToOriginal: string;
  btnTopUpCredits: string;
  btnEnterReferralCode: string;
  btnSelectTemplate: string;
  btnShare: string;

  // Categories
  category3DFigurine: string;
  categoryPortraitStyling: string;
  ...
  // Admin (optional - can stay Korean for now)
  adminOnly: string;
}
...
export const messages: Record<Language, Messages> = {
  ko: {
    ...
    aiAnalyzing: (title: string, description: string) =>
      `🎨 **AI 추천으로 편집 중...**\n\n✨ ${title}\n${description}\n\n⏳ 잠시만 기다려주세요...`,
    imageProcessing: (templateName: string) =>
      `🎨 **이미지 편집 중...**\n\n✨ ${templateName}\n\n⏳ 30초 정도 소요됩니다...`,
    ...
    creditsBalance: (free: number, paid: number, subscription: number, total: number) =>
      `💳 **크레딧 잔액**\n\n🎁 무료: ${free}회\n💰 충전: ${paid}회\n⭐ 구독: ${subscription}회\n\n**총 ${total}회** 사용 가능`,
    creditsLow: (current: number, required: number) =>
      `⚠️ 크레딧이 부족합니다.\n\n현재 잔액: ${current}회\n필요한 크레딧: ${required}회`,
    ...
    referralCode: (code: string) => `📋 **내 추천 코드:** \`${code}\``,
    referralLink: (link: string) => `🔗 **초대 링크:**\n${link}`,
    referralStats: (invites: number, credits: number) =>
      `📊 **초대 통계**\n👥 총 초대: ${invites}명\n💰 획득 크레딧: ${credits}회`,
    referralNextMilestone: (milestone: string, current: number, target: number) =>
    ...
    categoryNotFound: (category: string) => `❌ ${category} 카테고리를 불러오는 중 오류가 발생했습니다.`,
  ...
  en: {
    ...
    aiAnalyzing: (title: string, description: string) =>
      `🎨 **Editing with AI recommendation...**\n\n✨ ${title}\n${description}\n\n⏳ Please wait...`,
    imageProcessing: (templateName: string) =>
    ...
    referralCode: (code: string) => `📋 **My referral code:** \`${code}\``,
    referralLink: (link: string) => `🔗 **Invitation link:**\n${link}`,
    referralStats: (invites: number, credits: number) =>
    ...
    categoryNotFound: (category: string) => `❌ Error loading ${category} category.`,
