...
export class ImageAnalyzer implements IImageAnalyzer {
  ...
  async analyze(imageBuffer: Buffer): Promise<ImageAnalysis> {
    ...
    try {
      ...
      const suggestedCategories = this.suggestCategories(
        faceAnalysis,
        objectDetection,
        sceneAnalysis
      );
   ...
   */
  async analyzeMultiple(imageBuffers: Buffer[]): Promise<ImageAnalysis> {
    const startTime = Date.now();

    // Analyze each image
    const analyses = await Promise.all(
      imageBuffers.map(buffer => this.analyze(buffer))
    );
   ...
   */
  private async analyzeFaces(imageBase64: string): Promise<FaceAnalysis> {
...
Return as JSON format:
{
  ...
  "faces": [
    ...
    try {
      ...
      const faceAnalysis: FaceAnalysis = {
        ...
        positions: (result.faces || []).map((face: any) => ({
          x: face.x || 0,
          y: face.y || 0,
          width: face.width || 0,
        ...
        expressions: result.faces?.map((f: any) => f.expression).filter(Boolean),
        ages: result.faces?.map((f: any) => f.age).filter(Boolean),
        genders: result.faces?.map((f: any) => f.gender).filter(Boolean)
      };
   ...
   */
  private async detectObjects(imageBase64: string): Promise<DetectedObject[]> {
...
Return as JSON:
{
   ...
   * Analyze composition
   */
  private async analyzeComposition(imageBase64: string): Promise<CompositionAnalysis> {
...
Return as JSON:
{
   ...
   */
  private async analyzeQuality(imageBuffer: Buffer): Promise<QualityMetrics> {
    ...
    if (sizeInKB > 500) overallScore = 0.7;
    if (sizeInKB > 1000) overallScore = 0.8;
    if (sizeInKB > 2000) overallScore = 0.9;
   ...
   */
  private async analyzeDominantColors(imageBase64: string): Promise<string[]> {
  ...
  private suggestCategories(
    ...
    const hasClothing = objects.some(obj =>
  ...
  private calculateConfidenceScores(
    ...
    if (faces.count === 1) scores.portrait_styling += 0.4;
    if (faces.clarity === 'high') scores.portrait_styling += 0.3;
    if (quality.overallScore > 0.7) scores.portrait_styling += 0.3;
   ...
   */
  private async callGeminiVision(imageBase64: string, prompt: string): Promise<string> {
   ...
   */
  private mergeAnalyses(analyses: ImageAnalysis[]): ImageAnalysis {
