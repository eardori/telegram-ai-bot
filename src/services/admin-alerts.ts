...
import { SystemAlert, AlertType } from '../types/admin.types';
...
export async function getSystemAlerts(): Promise<SystemAlert[]> {
  ...
  if (apiCostAlert) alerts.push(apiCostAlert);
 ...
 * Check API error rate
 */
async function checkAPIErrorRate(): Promise<SystemAlert | null> {
  try {
 ...
 * Check daily API cost
 */
async function checkAPICost(): Promise<SystemAlert | null> {
  try {
 ...
 * Check payment failures
 */
async function checkPaymentFailures(): Promise<SystemAlert | null> {
  try {
    const last24h = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
 ...
 * Check revenue drop
 */
async function checkRevenueDrop(): Promise<SystemAlert | null> {
  try {
    ...
    const { data: prev7d } = await supabase
      .from('credit_transactions')
      .select(`
        *,
        credit_packages(price_stars),
 ...
 */
export function formatAlertsMessage(alerts: SystemAlert[]): string {
  if (alerts.length === 0) {
    return '✅ 시스템 정상 작동 중';
  }
