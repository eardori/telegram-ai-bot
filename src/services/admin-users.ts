...
import { UserInfo } from '../types/admin.types';
...
export function formatUserInfo(user: UserInfo): string {
  ...
  if (user.firstName) message += `• 이름: ${user.firstName}`;
  ...
  // Credits
  message += `💳 **크레딧 현황:**\n`;
  if (user.credits.free > 0) message += `• 🎁 무료 크레딧: ${user.credits.free}회\n`;
  if (user.credits.paid > 0) message += `• 💰 충전 크레딧: ${user.credits.paid}회\n`;
  if (user.credits.subscription > 0) message += `• ⭐ 구독 크레딧: ${user.credits.subscription}회\n`;
  ...
  // Flags
  message += `🎯 **상태:**\n`;
  if (user.flags.isVip) message += `• ⭐ VIP 사용자\n`;
  if (user.flags.isBanned) message += `• 🚫 제재됨\n`;
