...
export class TemplateMatcher implements ITemplateMatcher {
   ...
   */
  async matchTemplates(analysis: ImageAnalysis): Promise<PromptTemplate[]> {
    // Ensure templates are loaded
    await this.loadTemplates();

    // Filter compatible templates
    const compatibleTemplates = this.templates.filter(template =>
      this.isCompatible(template, analysis)
    );
   ...
   */
  private isCompatible(template: PromptTemplate, analysis: ImageAnalysis): boolean {
   ...
   */
  private calculateBonusScore(template: PromptTemplate, analysis: ImageAnalysis): number {
    let bonus = 0;
   ...
   */
  private async loadTemplates(forceRefresh: boolean = false): Promise<void> {
    const now = Date.now();
   ...
   */
  private mapDatabaseTemplate(row: any): PromptTemplate {
    return {
      id: row.id,
   ...
   */
  private getHardcodedTemplates(): PromptTemplate[] {
    ...
    return [
      {
        ...
        category: PromptCategory.THREE_D_FIGURINE,
      ...
      },
      {
        ...
        category: PromptCategory.IMAGE_EDITING,
      ...
      },
      {
        ...
        category: PromptCategory.PORTRAIT_STYLING,
        subcategory: 'glamour',
   ...
   * Get template by key
   */
  async getTemplateByKey(templateKey: string): Promise<PromptTemplate | null> {
    await this.loadTemplates();
   ...
   */
  async updateTemplateStats(templateId: number, success: boolean, processingTimeMs?: number): Promise<void> {
    try {
      // Update usage count and success rate
      const { error } = await supabase.rpc('update_template_stats', {
