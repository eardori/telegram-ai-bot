 ...
 * 추천 코드로 사용자 ID 조회
 */
export async function getUserIdByReferralCode(referralCode: string): Promise<number | null> {
  try {
    const { data: user, error } = await supabase
      .from('users')
      .select('id')
  ...
  try {
    ...
    if (milestones) {
      ...
      const next = milestones.find(m =>
        !achievedIds.has(m.id) && m.required_referrals > (stats.total_referrals || 0)
      );
 ...
 * 사용자가 달성한 마일스톤 조회
 */
export async function getUserMilestones(userId: number): Promise<number[]> {
  try {
 ...
 * 추천 메시지 포맷팅
 */
export function formatReferralMessage(stats: ReferralStats, botUsername: string): string {
 ...
 * 추천 성공 메시지 (추천인용)
 */
export function formatReferrerNotification(referredUsername: string, reward: number): string {
  return `🎉 **새로운 친구 가입!**\n\n` +
 ...
 * 추천 성공 메시지 (피추천인용)
 */
export function formatReferredWelcome(reward: number): string {
  return `🎊 **가입을 환영합니다!**\n\n` +
 ...
 * 마일스톤 달성 메시지
 */
export function formatMilestoneAchievement(
  milestoneName: string,
  bonusCredits: number,
  specialReward?: string
): string {
